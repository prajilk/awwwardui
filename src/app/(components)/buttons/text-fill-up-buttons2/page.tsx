...
const usageCode = `import TextFillUpButton2 from "@/components/ui/text-fill-up-button2";

const App = () => {
  return (
    <TextFillUpButton2
      classNames={{ innerWrapper: "bg-black text-white" }}
      size={"lg"}
    >
      Go to home
    </TextFillUpButton2>
...
const WavyButtonsPage = () => {
  const tsCode = readFileAsString(
    path.resolve("./src/components/buttons/text-fill-up-button2.tsx"),
  );
  const jsCode = readFileAsString(
    path.resolve("./src/lib/code/text-fill-up-button2.jsx"),
  );
  ...
  return (
    <div>
      ...
      <div className="grid place-items-center gap-10 rounded-md bg-white py-10">
        <TextFillUpButton2 size={"sm"}>Go to home</TextFillUpButton2>
      ...
      <CodeViewer
        jsCode={jsCode}
        tsCode={tsCode}
        filePath="components/ui/text-fill-up-button2.tsx"
      />
    </div>
  );
};
